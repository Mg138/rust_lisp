...
pub fn eval(env: Arc<Mutex<Env>>, expression: &Value) -> Result<Value, RuntimeError> {
...
fn eval_inner(
    ...
    match expression {
        // look up symbol
        Value::Symbol(symbol) => env.lock().unwrap().get(symbol).ok_or_else(|| RuntimeError {
        ...
        Value::List(list) if *list != List::NIL => {
            match &list.car()? {
                ...
                Value::Symbol(Symbol(keyword)) if keyword == "define" || keyword == "set" => {
                    ...
                    if keyword == "define" {
                        env.lock().unwrap().define(symbol.clone(), value.clone());
                    } else {
                        env.lock().unwrap().set(symbol.clone(), value.clone())?;
                    }
                ...
                Value::Symbol(Symbol(keyword)) if keyword == "defmacro" => {
                    ...
                    let body = Arc::new(Value::List(list.cdr().cdr().cdr()));
                    ...
                    env.lock().unwrap().define(symbol.clone(), lambda);
                ...
                Value::Symbol(Symbol(keyword)) if keyword == "lambda" => {
                    ...
                    let argnames = value_to_argnames(argnames_list.clone())?;
                    let body = Arc::new(Value::List(list.cdr().cdr()));
                ...
                Value::Symbol(Symbol(keyword)) if keyword == "let" => {
                    let let_env = Arc::new(Mutex::new(Env::extend(env)));
                    ...
                    for decl in declarations.into_iter() {
                        ...
                        let_env.lock().unwrap().define(symbol.clone(), result);
                    }
